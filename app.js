...
/* IDهای قدیمی اشتباه (قبل از کشف ID واقعی سایت از باندل freemodels.pro) → ID درست.
   اگر ID ناشناس به آپستریم برود، بی‌سروصدا به مدل پیش‌فرض (Claude) fallback می‌کند! */
...
/** تطبیق نرم نام مدل: «GPT 5.6 Sol» یا ID قدیمی «gpt-5.6-sol» یا «claude_fable 5.1» هم پذیرفته می‌شود */
function resolveModelId(input) {
  ...
  const canonical = LEGACY_MODEL_ALIASES[t] || t;
  const hit = FM_MODELS.find((m) => m.id === canonical || m.name.toLowerCase() === input.trim().toLowerCase());
...
function loadSettings() {
  try {
    ...
    if (raw) {
      ...
      /* IDهای قدیمی اشتباه → ID درست (مدل انتخابی قبلی کاربر بدون‌خطا منتقل می‌شود) */
